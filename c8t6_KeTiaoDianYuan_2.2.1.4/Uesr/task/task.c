        ...
        else
        {
            ...
            else
            {
                ...
                case Overrun:
                    ...
                    if(OutputCompleteCountdown != 0)
                    {
                        Restart_Output();  //尝试重新输出
                        OutputCompleteCountdown = 0;  //停止计时
                    }
                    break;
...
void Check_Setting(void)
{
    ...
    if((FieldTmp & 0x1000) == 0x0000)
    {
        Set_SC8701_GearFlag(SC8701_GEAR_SINGLE);
        SC8701_CE_Control(CE_DESABLE);
        Delay_ms(300);
        SC8701_FB_Control(FB_HIGH);
    }
    else if((FieldTmp & 0x1000) == 0x1000)
    {
        Set_SC8701_GearFlag(SC8701_GEAR_DOUBLE);
    }
}
