...
static const char VersionNumber[] = "2.2.1.4";  //版本号：前两位为硬件后两位为软件
...
uint16_t OutputCompleteCountdown;   //电压输出完成倒计时，倒计时内正常输出则不重启输出
...
static void MenuShow_Gear(void);
...
static void Menu_Set_Gear(uint8_t direction)
{
    uint16_t FieldTmp = Read_SettingField();

    OLED_ShowString(4, 1, "                ");
    OLED_ShowChinese(4, 1, 32);  //选
    OLED_ShowChinese(4, 3, 33);  //项
    OLED_ShowChar(4, 5,  ':');
    ...
    if(direction == LEFT)
    {
        ...
        OLED_ShowChinese(4, 11, 38);  //单
        OLED_ShowChinese(4, 13, 34);  //档
        OLED_ShowChinese(4, 15, 35);  //位
    }
    else if(direction == RIGHT)
    {
        FieldTmp |= 0x1000;
        ...
        OLED_ShowChinese(4, 11, 39);  //双
        OLED_ShowChinese(4, 13, 34);  //档
        OLED_ShowChinese(4, 15, 35);  //位
    }
    else if(direction == ONLY_SHOW)
    {
        switch (FieldTmp & 0x1000)
        {
        case 0x0000:
            OLED_ShowChinese(4, 11, 38);  //单
            OLED_ShowChinese(4, 13, 34);  //档
...
static void MenuShow_Gear(void)
{
    ...
    OLED_ShowChar(3, 1,  '<');
    OLED_ShowChar(3, 16, '>');
    OLED_ShowChinese(3, 5, 34);
    OLED_ShowChinese(3, 7, 35);
    OLED_ShowChinese(3, 9, 36);
    OLED_ShowChinese(3, 11, 37);
}
...
static void Node_LeftMove(void)
{
    ...
    {
        if(InPutVoltage == 5 && VolCursor == 11)  //5V输入下不允许大跨度降压操作
        {
...
static void Node_Subtract(void)
{
    ...
    if(VolCurSetingFlag == 0)  //电压
    {
        ...
        else if(VolCursor == 11 && VolRestrict >= (MINIMUM_VOL + 100))
        {
            ...
            if(VolRestrict >= 1500 && InPutVoltage < 20)
            {
                SlowFlag = 1;
            }

            if(VolRestrict >= 2500 && InPutVoltage == 20)
            {
                SlowFlag = 1;
            }
        }
    ...
    if(Get_SC8701_GearFlag() == SC8701_GEAR_SINGLE && VolRestrict < 600)  //单挡位硬件限制，应 > 6V
    {
        VolRestrict = 600;
        OLED_ShowString(3, 1, "                ");
        OLED_ShowString(4, 1, "                ");
        OLED_ShowChinese(4, 5, 40);  //需
        OLED_ShowChinese(4, 7, 39);  //双
        OLED_ShowChinese(4, 9, 34);  //档
        OLED_ShowChinese(4, 11, 35);  //位
    ...
    //显示
    if(SlowFlag || Get_SC8701_GearFlag() == SC8701_GEAR_SINGLE)  //因为显示进度条被擦除了
    {
        OLED_ShowChinese(3, 1, 7);
        OLED_ShowChinese(3, 3, 8);
        OLED_ShowChinese(3, 5, 17);
...
static void Node_Add(void)
{
    ...
    if(VolCurSetingFlag == 0)  //电压
    {
        ...
        else if(VolCursor == 11 && VolRestrict <= (2400 + ExtraVol))
        {
            ...
            //补丁：修复5V输入状态下无法通过滚轮升压超过10V
            if(InPutVoltage == 5 && (VolRestrict - 100) <= 1000 && SC8701_CE_Read() == CE_ENABLE)
            {
                SC8701_CE_Control(CE_DESABLE);
                Delay_ms(300);
                SC8701_CE_Control(CE_ENABLE);
        ...
        else if(VolCursor == 13 && VolRestrict <= (2490 + ExtraVol))
        {
            ...
            //补丁：修复5V输入状态下无法通过滚轮升压超过10V
            if(InPutVoltage == 5 && (VolRestrict - 10) <= 1000 && SC8701_CE_Read() == CE_ENABLE)
            {
                SC8701_CE_Control(CE_DESABLE);
                Delay_ms(300);
                SC8701_CE_Control(CE_ENABLE);
        ...
        else if(VolCursor == 14 && VolRestrict <= (2499 + ExtraVol))
        {
            ...
            //补丁：修复5V输入状态下无法通过滚轮升压超过10V
            if(InPutVoltage == 5 && (VolRestrict - 1) <= 1000 && SC8701_CE_Read() == CE_ENABLE)
            {
                SC8701_CE_Control(CE_DESABLE);
                Delay_ms(300);
                SC8701_CE_Control(CE_ENABLE);
...
static void Node_ON_OFF_OutPut(void)
{
    ...
    {
        if(VolRestrict < 600 && Get_SC8701_GearFlag() == SC8701_GEAR_SINGLE)
        {
            OLED_ShowChinese(4, 5, 40);  //需
            OLED_ShowChinese(4, 7, 39);  //双
            OLED_ShowChinese(4, 9, 34);  //档
        ...
        else
        {
            ...
            OutputCompleteCountdown = 350;  //开始倒计时
        }
    }
...
static void Node_QuitPredefined(void)
{
    ...
    if(Predefined_OutPut_Array[PredefinedCount].PreVol < 600 && Get_SC8701_GearFlag() == SC8701_GEAR_SINGLE)
    {
        OLED_ShowChinese(4, 5, 40);  //需
        OLED_ShowChinese(4, 7, 39);  //双
        OLED_ShowChinese(4, 9, 34);  //档
...
static void Node_MenuOption_Left(void)
{
    ...
    {
        EnumFlag = (ENUM_OPTION_ID)(ENUM_OPTION_NUM - 1);
    }
